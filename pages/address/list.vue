	...
	<view class="container">
		<view class="address-list">
			<view class="a-address" v-for="(item, index) in addressList" :key="index">
				...
				<view @tap="deleteAddess" class="right-delete" :data-id="item.id"><image class="deleteImg" src="/static/images/delete@2x.png"></image></view>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		selectTap: function(e) {
			...
			this.$api.user.addressDefault.request({ id: s }).then(data => {
				// TODO 要修改
		...
		deleteAddess: function(e) {
			...
			uni.showModal({
				...
				success: function(e) {
					if (e.confirm) {
						that.$api.user.addressDelete.request({ id: id }).then(data => {
